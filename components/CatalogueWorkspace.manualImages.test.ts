...
// --- Bulk Upload's own multi-image CSV columns are covered in
// CatalogueWorkspace.bulkUploadMultiImage.test.ts (Milestone C17.2) ---------

// Milestone C18 revises this: Photos Only is no longer untouched overall
// (it gained multi-select/grouping: see CatalogueWorkspace.imageGrouping
// .test.ts), but its ORIGINAL single-image path (imageFile/onImageFileChange,
// unchanged from C1-era Photos Only) must still be exactly what it always
// was for the one-file case, and it must still never reach for Manual
// Entry's OWN multi-image machinery (manualImages/ManualEntryImages/
// MAX_MANUAL_IMAGES belong to a different cap/UI entirely: C18's own
// MAX_IMAGE_GROUPING_BATCH is a separate, per-import concept, not a reuse
// of Manual Entry's per-product cap).
test('Photos Only single-image path is unchanged; it never reuses Manual Entry\'s own multi-image machinery', () => {
